...
void ContrastiveLossLayer<Dtype>::Forward_gpu(
  ...
  for (int i = 0; i < bottom[0]->num(); ++i) {
    ...
    if (static_cast<int>(bottom[2]->cpu_data()[i]) ==
        static_cast<int>(bottom[3]->cpu_data()[i])) {  // similar pairs
...
__global__ void CLLBackward(const int count, const int channels,
    ...
    const Dtype* y0, const Dtype* y1, const Dtype* diff, const Dtype* dist_sq,
    Dtype *bottom_diff) {
  ...
  for (int i = 0; i < 2; ++i) {
    if (propagate_down[i]) {
      ...
      CLLBackward<Dtype><<<CAFFE_GET_BLOCKS(count), CAFFE_CUDA_NUM_THREADS>>>(
          ...
          bottom[3]->gpu_data(),  // pair similarity 0 or 1
